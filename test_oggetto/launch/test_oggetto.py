...
def generate_launch_description():
    package_dir = get_package_share_directory('test_oggetto')
    ...
    oggetto_description = pathlib.Path(os.path.join(package_dir, 'resource', 'Beep.urdf')).read_text()

    webots = WebotsLauncher(
        world=os.path.join(package_dir, 'worlds', 'test_oggetto.wbt'),
    ...
    oggetto_driver = WebotsController(

        # robot_name= 'TIAGo%20Base',
        robot_name= 'Beep',
        parameters=[
            {'robot_description': oggetto_description},
            {'use_sim_time': True},
        ] 
