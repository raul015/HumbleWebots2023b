...
illuminazione = 0.0 # Valore di defaul della illuminazione.
apertura = 0.5  #Valore di inizializzazione apertura porta
...
class TIAGODriver:
    ...
    def init(self, webots_node, properties): 
       ...
       # Inserimento di una telecamera
        ...
        for k in range(self.n):
            ...
            if(nodo_k.getTypeName() == 'Door'):    # Cerco il nodo Door in base al nome
                
                self.__node.get_logger().info('trovato Door alla posizione  '+ str(k))
                door_position = nodo_k.getField('position')
                door_position.setSFFloat(apertura)
    ...
    def step(self): 
        ...
        global command_motor_left, command_motor_right, forward_speed,angular_speed,illuminazione,apertura
        messaggio_ir_right, messaggio_ir_left, messaggio_distanza_middle, messaggio_light
        ...
        for k in range(self.n):
            ...
            if(self.root_children_field.getMFNode(k).getTypeName() == 'PointLight'):  # Cerco il nodo PointLihgt in base al nome
            ...
            if(self.root_children_field.getMFNode(k).getTypeName() == 'Door'):
                
                # self.__node.get_logger().info('trovato un PointLight alla posizione  '+ str(k))
                prova_door = self.root_children_field.getMFNode(k)
                campo_door = prova_door.getField('position')
                campo_door.setSFFloat(apertura) #
                
                # self.__node.get_logger().info('trovato un Door alla posizione  '+ str(k))
