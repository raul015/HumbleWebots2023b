...
def generate_launch_description():
    package_dir = get_package_share_directory('test_uomo')
    ...
    uomo_description = pathlib.Path(os.path.join(package_dir, 'resource', 'Uomo.urdf')).read_text()

    webots = WebotsLauncher(
        world=os.path.join(package_dir, 'worlds', 'test_uomo.wbt'),
    ...
    uomo_driver = WebotsController(

        # robot_name= 'TIAGo%20Base',
        robot_name= 'pedestrian',
        parameters=[
            {'robot_description': uomo_description},
            {'use_sim_time': True},
        ] 
